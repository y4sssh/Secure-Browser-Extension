    ...
    def do_GET(self):
        ...
        if p.path == "/":
            ...
            html = (
                ...
                "  try {"
                "    // post to FastAPI backend running on port 8002"
                "    const r = await fetch('http://127.0.0.1:8002/api/v1/evidence', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });"
                ...
                "  return fetch('http://127.0.0.1:8002/api/v1/analyze/url', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: url, redirectChain: [] }) }).then(r=>r.json());"
                "}"
                "const analyzeBtn = document.createElement('button');"
                "analyzeBtn.textContent = 'Analyze current URL';"
                "analyzeBtn.type = 'button';"
                "analyzeBtn.addEventListener('click', async ()=>{"
                "  const url = document.getElementById('hostname').value.startsWith('http') ? document.getElementById('hostname').value : 'http://' + document.getElementById('hostname').value;"
                "  try{ const res = await analyzeUrl(url); document.getElementById('resp').textContent = JSON.stringify({ analyze: res }, null, 2); } catch(e){ document.getElementById('resp').textContent = String(e); }"
                "});"
                "document.getElementById('evidenceForm').appendChild(analyzeBtn);"
                "</script>"
                "</body></html>"
            )
